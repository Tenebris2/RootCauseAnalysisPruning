...
def extract():
    ...
    total_results_from_running_test_cases = [
        [
            "Test Case",
            "Method",
            "ID",
    ...
    for dirpath, dirnames, filenames in os.walk(RESULTS):
        if len(filenames) != 0 and len(dirnames) == 0:
            row = []
            method, id = (
                " ".join(dirpath.split("/")[-1].split("_")[:-1]),
                dirpath.split("/")[-1].split("_")[-1],
            )
            ...
            for f in filenames:
                if f == "rca_results.txt":
                    ...
                    instructions_traced, total_instructions = collect_hitcount(
                        hitcount_results_path
                    )
                    ...
                    prepare_time_path = dirpath + "/" + f
                    time = collect_time_to_extract_address_from_source(
                        prepare_time_path
                    )
...
def collect_rca_results(file_path):
    with open(file_path, "r") as file:
        ...
        ranking_time = ""
        for line in lines:
            match_crashes_and_non_crashes = re.search(
                CRASHES_AND_NON_CRASHES_INDICATOR, line
            )
